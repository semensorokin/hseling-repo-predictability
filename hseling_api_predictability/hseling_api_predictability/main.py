...
from hseling_api_predictability import boilerplate

from hseling_lib_predictability.process import process_data
from hseling_lib_predictability.query import query_data


app = Flask(__name__)
app.config['DEBUG'] = False
app.config['LOG_DIR'] = '/tmp/'
app.config.from_envvar('HSELING_API_PREDICTABILITY_SETTINGS')
...
if not app.debug:
    ...
    file_handler = TimedRotatingFileHandler(os.path.join(app.config['LOG_DIR'], 'hseling_api_predictability.log'), 'midnight')
...
def healthz():
    ...
    return jsonify({"status": "ok", "message": "Application Predictability"})
